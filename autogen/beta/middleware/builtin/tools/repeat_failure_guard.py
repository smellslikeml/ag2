...
class RepeatToolFailureError(Exception):
    """Carried on the :class:`~autogen.beta.events.ToolErrorEvent` returned when a
...
def repeat_failure_guard(*, block: bool = True) -> ToolMiddleware:
    ...
    :class:`~autogen.beta.events.ToolErrorEvent`) or, in annotate mode, lets
    ...
    async def guard(
        ...
        if key in log and block:
            return ToolErrorEvent.from_call(
                event,
                error=RepeatToolFailureError(f"Blocked by repeat-failure guard: prior failure `{log[key][:80]}`"),
            )
