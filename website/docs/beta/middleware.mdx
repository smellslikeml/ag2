...
AG2 Beta currently includes five built-in middleware in `autogen.beta.middleware`:
...
agent = Agent(..., middleware=[ContextElasticizer(raw_steps=4)])
```

Adapts the event history before each model call by assigning every step one of three elastic types: the most recent `raw_steps` steps pass through unchanged, older steps that still fit the abstraction budget are collapsed into compact digests, and the oldest steps beyond the budget are dropped from the view sent to the model.

This is reversible: the elasticizer only ever builds the view handed to the model — it never mutates the agent's stream, so the raw events behind every abstracted or dropped step remain available losslessly.
It also caches the raw events behind each digest it emits and exposes them through `expand(...)`.

Use it when `HistoryLimiter` or `TokenLimiter` are too coarse — you want older context kept reachable as a digest rather than discarded outright.
